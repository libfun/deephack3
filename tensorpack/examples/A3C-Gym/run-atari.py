...
def get_player(seed=None, dumpdir=None):
    pl = GymEnv(ENV_NAME, dumpdir=dumpdir, auto_restart=False, seed=seed)
...
def run_submission(cfg, output, nr, seed):
    player = get_player(seed=seed, dumpdir=output)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', help='output directory', default=0)
    ...
    cfg = PredictConfig(
        ...
        input_names=['state'],
        output_names=['logits'])
    run_submission(cfg, args.output, args.episode, int(args.seed))
